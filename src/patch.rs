...
use crate::defs::{END_MARKER_PREFIX, START_MARKER_PREFIX};
    ...
    fn render(&self) -> Option<String> {
        match self {
            ...
            Block::Managed { name, content } => {
                let mut s = format!("{START_MARKER_PREFIX}{name}\n");
                let trimmed = content.trim();
                if !trimmed.is_empty() {
                    s.push_str(trimmed);
                    s.push('\n');
                }
                s.push_str(END_MARKER_PREFIX);
                s.push_str(name);
                s.push('\n');
                Some(s)
            }
...
impl Patcher {
    ...
    pub fn parse(content: &str) -> Self {
        ...
        while let Some(line) = lines.next() {
            ...
            if let Some(name) = line.strip_prefix(START_MARKER_PREFIX) {
                if !buffer.is_empty() {
                    blocks.push(Block::Custom(buffer.clone()));
                    buffer.clear();
                }
        ...
        if !buffer.is_empty() {
            blocks.push(Block::Custom(buffer));
        }
    ...
    pub fn patch(&mut self, new_templates: HashMap<String, String>) {
        ...
        self.blocks.extend(
            templates
                .into_values()
                .map(|(name, content)| Block::Managed {
                    name,
...
mod tests {
    ...
    fn test_patch_existing_template() {
        let content = format!(
            "{START_MARKER_PREFIX}Rust\nold_content\n{END_MARKER_PREFIX}Rust\n"
        );
